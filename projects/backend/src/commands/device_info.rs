...
use crate::services::device_sources::{
    fetch_from_all_sources, merge_results, DeviceSource, WikipediaSource,
};
use crate::services::{
    AiAgent, CacheManager, ClaudeClient, ImageCache, InternetFetcher, KnowledgeStore,
...
fn get_cache_manager() -> &'static CacheManager {
    CACHE_MANAGER.get_or_init(|| CacheManager::new().expect("Failed to initialize CacheManager"))
}
...
fn get_internet_fetcher() -> &'static InternetFetcher {
    INTERNET_FETCHER
        .get_or_init(|| InternetFetcher::new().expect("Failed to initialize InternetFetcher"))
}

/// Get or initialize the AI agent.
fn get_ai_agent() -> &'static AiAgent {
    AI_AGENT.get_or_init(|| AiAgent::new().expect("Failed to initialize AiAgent"))
}

/// Get or initialize the Claude client.
fn get_claude_client() -> &'static ClaudeClient {
    CLAUDE_CLIENT.get_or_init(|| ClaudeClient::new().expect("Failed to initialize ClaudeClient"))
}

/// Get or initialize the knowledge store.
fn get_knowledge_store() -> &'static KnowledgeStore {
    KNOWLEDGE_STORE
        .get_or_init(|| KnowledgeStore::new().expect("Failed to initialize KnowledgeStore"))
}

/// Get or initialize the image cache.
fn get_image_cache() -> &'static ImageCache {
    IMAGE_CACHE.get_or_init(|| ImageCache::new().expect("Failed to initialize ImageCache"))
}
...
pub async fn get_device_deep_info(
    ...
    log::debug!(
        "Parsed identifier: manufacturer={}, model={}",
    ...
    if !sources.is_empty() {
        ...
        log::info!(
            "Multi-source fetch: {} sources returned data",
            successful_count
        );

        if let Some(merged) = merge_results(results) {
            log::info!(
                "Successfully merged data from sources: {}",
                merged.source_name
            );
    ...
    match fetcher.fetch_device_info(&identifier, &device_type).await {
        Ok(mut web_info) => {
            log::info!(
                "Successfully fetched device info from web for: {}",
                device_id
            );
...
pub fn clear_device_cache(
    ...
    match (device_id, device_type) {
        (Some(id), Some(dt)) => cache.remove(&id, &dt).map_err(|e| e.to_string()),
        _ => cache.clear().map_err(|e| e.to_string()),
    }
}
...
pub async fn fetch_device_image_with_key(
    ...
    match image_cache.fetch_and_cache_with_key(&url, &cache_key).await {
        ...
        Err(e) => {
            log::warn!(
                "Failed to fetch image {} with key {}: {}",
                url,
                cache_key,
                e
            );
...
fn get_enrichment_service() -> &'static DeviceEnrichmentService {
    ENRICHMENT_SERVICE.get_or_init(|| {
        ...
        let image_cache =
            Arc::new(ImageCache::new().expect("Failed to create ImageCache for enrichment"));
